...
class MassItemInventorySpec extends FlatSpec with Matchers {
  ...
  it should "not accept an item of a different type than what's stored inside" in {
    val inventory = new MassItemInventory(stackType = new ItemStack(TestItems.Red), amountStored = 1)
  ...
  it should "not accept items past its capacity" in {
    val inventory = new MassItemInventory(stackType = new ItemStack(TestItems.Red), maxStacks = 2, amountStored = 64 + 63)
  ...
  it should "allow a new item type when emptied and set to accept new stacks" in {
    val inventory = new MassItemInventory(stackType = new ItemStack(TestItems.Red), amountStored = 1, allowNewStackWhenEmpty = true)
  ...
  it should "not allow a new item type when emptied and not set to accept new stacks" in {
    val inventory = new MassItemInventory(stackType = new ItemStack(TestItems.Red), amountStored = 1, allowNewStackWhenEmpty = false)
  ...
  it should "only start to fill its input slot once it's within the last stack of its capacity" in {
    val inventory = new MassItemInventory(stackType = new ItemStack(TestItems.Red), amountStored = 64, maxStacks = 2)
  ...
  it should "lower its stored amount when stacks are taken" in {
    val inventory = new MassItemInventory(stackType = new ItemStack(TestItems.Red), amountStored = 128)
